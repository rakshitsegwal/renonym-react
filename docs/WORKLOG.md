...
| 2026-06-10 | `src/coach/CoachCheckout.jsx`, `src/coach/InterviewSetup.jsx` | BROKEN FLOW: payment succeeds but nothing happens — post-payment error was silent + checkout reachable without a JD draft + setup looped to checkout on creation error | Fixed: prominent errors, no-draft→setup, no checkout loop, paid fallback button. Deployed. Awaiting retry to capture the underlying error. |
| 2026-06-10 | backend `server.js` verify-payment | Entitlement not saving (payment loop): grant ran only on body userId which was missing | Fixed: grant via JWT user id (v11.1), rowCount logging. Deployed. |
| 2026-06-10 | `src/coach/CoachCheckout.jsx` | Loop: silently returned to checkout when grant missing | Confirm via /coach/me after pay; clear message if not activated. |
| 2026-06-10 | `src/coach/InterviewSetup.jsx`, `src/coach/api.js` | Fake résumé (Maya_Chen_PM mock + dead upload + prefilled Stripe/JD) | Real résumé upload (pdf/docx/txt→extract-resume), saved-résumé reuse, mock removed, fields blank | Done, deployed |
